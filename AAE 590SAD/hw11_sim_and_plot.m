...
function [] = hw11_sim_and_plot(v_span, region, w_0, q_0)
    global K;
    
    % set plot formatting variables  
    ttl_start = ['Jordan Mayer, AAE 590/440, PS11, Problem 2, ', ...
                 'Numerical Results, '];
    ttl = [ttl_start, 'Region ', num2str(region)];
    ...
    if region == 1
        % orientation ii
        K = [-0.4706, -0.5571, 0.8142];
        fnum = 1;
    elseif region == 3
        % orientation iv
        K = [0.8142, -0.4706, -0.5571];
        fnum = 4;
    elseif region == 4
        % orientation i
        K = [0.5571, 0.4706, -0.8142];
        fnum = 2;
    elseif region == 6
        % orientation iii
        K = [-0.8142, 0.5571, 0.4706];
        fnum = 3;
    end
    ...
    if w_0(1) == 1.0 && w_0(2) == 0 && w_0(3) == 0
        ...
        [v, y] = ode45('hw11_numint_all', v_span, y_0, options);
    ...
    % plot nutation angle
    subplot(3,1,1); plot(v, sigma); hold on;
    if w_0(2) == 0.16
    ...
    subplot(3,1,3); plot(v, lambda); hold on;
    if w_0(2) == 0.16
        % last run; add formatting
        xlabel(xlab); ylabel('Inertial Precession Angle (sigma), deg'); grid on;
    end
end